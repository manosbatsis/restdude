...
@NoRepositoryBean
public interface ContextRepository<T extends MembershipContext> extends ModelRepository<T,String> {
    ...
    String SELECT_BUSINESS_CONTEXT_INFO =
            "select new com.restdude.domain.cases.model.dto.MembershipContextInfo(" + BUSINESSCONTEXTINFO_CONSTRUCTOR_PARAMS + ") from MembershipContext context ";
    String SELECT_COUNT_BUSINESS_CONTEXT_IDS = "select count(context.pk) from MembershipContext context ";
    ...
    Page<MembershipContextInfo> findVisibleMembershipMembershipContexts(User u, Pageable pageable);

    @Query(value = "select CASE WHEN COUNT(context) > 0 THEN true ELSE false END  "
            + "from MembershipContext context "
            + "  left join context.memberships as membership "
            + "where context.pk = ?1 "
    ...
    @Query("select (persisted.owner.pk = :#{#user.pk}) from MembershipContext persisted where persisted.pk = :#{#context.pk} ")
    boolean isOwner(@Param("context") MembershipContext context, @Param("user") User user);
    ...
    boolean canInviteUserToMembershipContext(@Param("context") MembershipContext context);
    ...
    @Query(value = "select CASE WHEN COUNT(membership) > 0 THEN true ELSE false END  "
            ...
            + "         or membership.context.owner.pk = ?#{principal.pk} ) ")
    boolean canApproveRequestToJoinMembershipContext(@Param("context") MembershipContext context);
    ...
    @Query(value = "select context from MembershipContext context  where context.owner.username = 'system' and context.name = '" + SYSTEM_CONTEXT_NAME + "' ")
