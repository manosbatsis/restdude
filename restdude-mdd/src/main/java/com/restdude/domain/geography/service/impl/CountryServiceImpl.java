...
public class CountryServiceImpl extends AbstractPersistableModelServiceImpl<Country, String, CountryRepository> implements CountryService {
    ...
    @Override
	@Transactional(readOnly = false)
    public void initContinentsAndCountries() {
        if (this.continentService.count() == 0) {
			log.debug("Initializing continents and countries");
			...
			log.debug("Continents and countries already initialized");
        }
    }
