...
//					u.setCreatedBy(system);
	...
	protected List<SystemError> initSystemErrors(List<User> users) {
		...
		for (int i = 0; i < stackTraces.length; i++) {
			...
			StackTrace st = this.stackTraceService.create(new StackTrace.Builder().id(stackTrace[0].toString()).rootCauseMessage(stackTrace[2].toString()).stacktrace(stackTrace[3].toString()).build());
